...
static unsigned int sqlctx = 2107;
...
36,0,0,2,0,0,30,43,0,0,0,0,0,1,0,
51,0,0,3,0,0,32,45,0,0,0,0,0,1,0,
66,0,0,4,24,0,1,90,0,0,0,0,0,1,0,
81,0,0,5,264,0,44,92,0,0,0,0,0,1,0,
};
...
int main(int argc, char** argv){
     /* VARCHAR msg_buf[51]; */ 
struct { unsigned short len; unsigned char arr[51]; } msg_buf;
     ...
     /* EXEC SQL DROP TABLE tEssai1 PURGE; */ 

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "drop TABLE tEssai1 PURGE";
     sqlstm.iters = (unsigned int  )1;
     ...
     /* EXEC SQL CREATE TABLE tEssai1(
          ...
          num NUMBER CHECK (num between 100 and 199),
          unCar char CHECK (unCar in ('A','B','C')),
          chaine VARCHAR2(10) CHECK (chaine like 'E%'),
          uneDate DATE CHECK (to_char(uneDate, 'yyyy') = '2017')
          ); */ 

{
     ...
     sqlstm.sqladtp = &sqladt;
     sqlstm.sqltdsp = &sqltds;
     sqlstm.stmt = "create TABLE tEssai1 ( id char ( 4 ) primary key , num N\
UMBER CHECK ( num between 100 and 199 ) , unCar char CHECK ( unCar in ( 'A' , \
'B' , 'C' ) ) , chaine VARCHAR2 ( 10 ) CHECK ( chaine like 'E%' ) , uneDate DA\
TE CHECK ( to_char ( uneDate , 'yyyy' ) = '2017' ) )";
     sqlstm.iters = (unsigned int  )1;
